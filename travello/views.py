...
def index(request):
    ...
    dest1 = []
    for i in range(6):
        j = (i + 1) * 2  # Adjust logic as needed
        try:
...
def login(request):
    if request.method == 'POST':
        ...
        # Authenticate using username (which is tied to the user's email)
        ...
        if user is not None:
            if user.is_active:  # Ensure the user is active
                ...
                # Optional: Send a notification email after successful login
                ...
                # send_mail('Alert for Login', content, 'travellotours89@gmail.com', [email], fail_silently=True)
